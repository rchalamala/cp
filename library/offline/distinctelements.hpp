...
{
	void dfs(Graph<BiEdge>& graph, vector<pair<size_t, size_t>>& times, ll& current, const size_t& i = 0, const size_t& parent = 0)
	{
		times[i].ff = ++current;
		for(const auto& neighbor : graph[i])
			if(neighbor.to != parent)
				dfs(graph, times, current, neighbor.to, i);
		times[i].ss = current;
	}
...
public:
	void solve(std::istream& in, std::ostream& out)
	{
		...
		for(size_t i = 0; i < n; ++i)
		{
			...
			{
				answer[get<2>(queries[index])] = bit.query(get<1>(queries[index]), get<0>(queries[index]));
				++index;
			}
		}
		prints(out, answer);
