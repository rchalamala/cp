...
class Modular {
 ...
 public:
  ...
  Modular(const T1& element) : value{normalize(element)} {
  ...
  template <typename T1>
  static Type normalize(const T1& element) {
    Type result{-modulus() <= element && element < modulus()
                    ? static_cast<Type>(element)
                    : static_cast<Type>(element % modulus())};
  ...
  template <typename T1>
  Modular& operator+=(const T1& other) {
  ...
  typename std::enable_if<(sizeof(typename Modular<T1>::Type) >= 8),
                          Modular&>::type&
  ...
  std::vector<Type> multiplicative_inverses(const Type& bound = modulus()) {
    ...
    for (std::size_t i = 2; i < bound; ++i)
      inverses[i] =
          (modulus() - (modulus() / i) * inverses[modulus() % i] % modulus()) %
          modulus();
    return inverses;
  }

  template <typename T1>
  friend std::istream& operator>>(std::istream&, Modular<T1>& element);
};
...
Modular<T1> operator-(Modular<T1> lhs, const T2& rhs) {
  return lhs -= rhs;
}

template <typename T1, typename T2>
Modular<T1> operator-(const T2& lhs, Modular<T1> rhs) {
  return rhs -= lhs;
}
...
template <typename T1, typename T2>
bool operator==(T2 lhs, const Modular<T1>& rhs) {
...
template <typename T>
std::istream& operator>>(std::istream& stream, Modular<T>& element) {
...
template <typename T>
std::ostream& operator<<(std::ostream& stream, const Modular<T>& element) {
