...
namespace factors
{
	...
	template<typename T> std::vector<T> optimized_rho_factorize(T n, const bool& checkBaseCases = true)
	{
		static_assert(std::is_integral_v<T>);
		assert(n >= 0);
		if(n <= 1)
		...
		n /= factor;
		std::vector<T> original{optimized_rho_factorize(factor, false)};
		...
		if(n > 1)
		{
			std::vector<T> divided{optimized_rho_factorize(n, false)};
			std::move(std::begin(divided), std::end(divided), std::back_inserter(original));
		}
		return original;
	}
